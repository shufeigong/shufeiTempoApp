import { type FC, type ReactElement, useEffect, useCallback, useRef, useState } from 'react';
...
export const BoardComponent:FC = ():ReactElement => {
    ...
    useEffect(() => {
        const syncTimer = setTimeout(async () => {
          ...
          try {
            ...
            console.log("Sync notes Complete!");
          } catch (err: any) {
    ...
    const updateNote = useCallback((id: string, updates: Partial<Note>):void => {
        //use useCallback hook to keep this arrow function reference stable.
            setNotes((prevNotes)=>{
                ...
                if(toUpdateNoteIndex < 0){return prevNotes;}
    ...
    const deleteNote = useCallback(async (id: string):Promise<void> => {
        ...
        setNotes((prevNotes) => {
            ...
            if (toDeleteNoteIndex < 0) { return prevNotes; }
    ...
    const focusNote = useCallback((id: string) => {
        ...
        setNotes((prevNotes) => {
          const maxZ = prevNotes.length > 0 
            ? Math.max(...prevNotes.map((note) => note.zIndex)) 
            : 0;
          
          const toUpdateNoteIndex = prevNotes.findIndex((note) => note.id === id);
          if (toUpdateNoteIndex < 0 || prevNotes[toUpdateNoteIndex].zIndex === maxZ) {return prevNotes;}
      ...
      return (
        <div className="board">
            ...
            <p className='board-header-text'>
                Created by React + TypeScript
            </p>
            </header>
            <div className="controls">
                <button onClick={addNote} className="add-btn">+ New Note</button>
            </div>
